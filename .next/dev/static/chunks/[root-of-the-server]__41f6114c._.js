...
class ZenGardenMoss {
    ...
    constructor(encoded, scene){
        ...
        this.mesh.position.set(encoded.position.x, 0.01, encoded.position.y);
        scene.add(this.mesh);
    }
    moveOnPlane(dx, dy) {
...
class ZenGardenEditor {
    ...
    handleMouseMove = (event)=>{
        ...
        const pos = this.scene.screenToPlaneCoordinate(screen.x, screen.y);
        if (pos) {
            const dx = pos.x - this.dragging.lastPos.x;
            const dy = pos.y - this.dragging.lastPos.y;
            this.dragging.object.moveOnPlane(dx, dy);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__41f6114c._.js.map